...
const SearchBank = () => {
  ...
  const searchBanks = useCallback(x => {
    setSearchText(x);
  ...
  return <View style={styles.container}>
      <Input text="Search" placeholder="Search for your bank" value={searchText} onChangeText={searchBanks} containerStyle={styles.inputContainer} icon={require("./assets/searchIcon.png")} />
      <View style={styles.separator}>
        <Text style={styles.separatorText}>Choose your bank</Text>
      </View>
      <FlatList data={searchResults.length > 0 ? searchResults : banks} renderItem={({
      item
    }) => <Pressable style={[styles.bankItem, item === selectedBank && styles.selectedBank]} onPress={() => setSelectedBank(item)}>
            <Image source={item.image} style={styles.bankImage} />
            <Text style={[styles.bankName, item === selectedBank && styles.selectedBank]}>
              {item.name}
            </Text>
